...
{
    int i = strlen(path);
    // declare string to hold index.php
    char* php = "index.php";
    // declare string to hold index.html
    char* ht = "index.html";
    // malloc string for returned path
    char* string = malloc(sizeof(char) * (i + strlen(php) +1));
    char* string2 = malloc(sizeof(char) * (i + strlen(ht) +1)); 
    // copy and attach strings 
    strcpy(string, path);
    strcat(string, php);
    strcpy(string2, path);
    strcat(string2, ht); 
...
{
    // allocated memory
    BYTE* adr = malloc(sizeof(BYTE));
    // number of bytes
    size_t num = 0;
    // read bytes till EOF
    while (fread(adr+num ,sizeof(BYTE),1,file) != 0)
    {
        num++;
        adr = realloc(adr, sizeof(BYTE)* (num+1));
    }
    // store bytes and number of bytes
    *content = adr;
    *length = num;
    return true;
}
